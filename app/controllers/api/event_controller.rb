class Api::EventController < ApiController
  ...
  def discover
    ...
    @featured_popups = Group.where.not(start_date: nil).where("group_tags @> ARRAY[?]::varchar[]", [ ":featured" ]).order(start_date: :desc)
    @popups = Group.where.not(start_date: nil).where.not("group_tags @> ARRAY[?]::varchar[]", [ ":hidden" ]).order(start_date: :desc)
