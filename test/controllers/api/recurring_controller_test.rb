  ...
  test "api#recurring/create" do
    ...
    assert_difference "Event.count", 5 do
      post api_recurring_create_url, params: {
        ...
        event_roles_attributes: [ { role: "cohost", item_type: "Profile", item_id: profile2.id } ]
      }
    end
    ...
    assert_equal [["cohost", "Profile", profile2.id]], recurring.events.order("start_time").first.event_roles.map { |er| [ er.role, er.item_type, er.item_id ] }
    ...
    post api_recurring_update_url,
      params: { auth_token: auth_token,
        ...
        event_roles_attributes: [ { role: "cohost", item_type: "Profile", item_id: profile3.id } ]
    }
    assert_response :success
    ...
    assert_equal [["cohost", "Profile", profile2.id], ["cohost", "Profile", profile3.id]], events_ordered.second.event_roles.map { |er| [ er.role, er.item_type, er.item_id ] }
